...
import cn.twopair.view.handler.TransferHandler;
...
public class TransferView extends JDialog {
    ...
    //备注组件
    private JLabel sendLabel;    //发送号码标签
    private JTextField sendText;    //发送号码文本域
    //底部
    ...
    public TransferView(MainView mainView) {

        super(mainView, "转账业务", true);
         TransferHandler  transferHandler = new TransferHandler(this, mainView);
        ...
        //创建"接收卡号"标签
        cardNumberLabel = new JLabel("accept：");
        SetIconUtil.setIcon(cardNumberLabel, "img/type.png");
        cardNumberLabel.setBounds(70, 30, 140, 40);    //设置"卡号"标签位置。
        ...
        SetIconUtil.setIcon(sendLabel, "img/remark.png");
        sendLabel.setBounds(90, 284, 160, 20);    //设置"备注"标签位置。
        sendLabel.setFont(new Font("隶书", Font.BOLD, 20));
        centerJPanel.add(sendLabel);    //添加"备注"标签。
        //创建"发送号码"文本域
        sendText = new JTextField(20);
        sendText.setBounds(175, 280, 260, 30);    //设置"备注"文本域位置。
        centerJPanel.add(sendText); //添加"备注"文本域
        ...
        BulkImportUtil.setFont(font, cardNumberText, moneyText, dateText,
                sendText, sureButton, cancelButton, cardPasswordText);
        ...
        //设置监听
        sureButton.addActionListener(transferHandler);
        cancelButton.addActionListener(transferHandler);
